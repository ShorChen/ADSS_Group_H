...
public class TransportController {
    ...
    public void addTruck(String licenseNumber, String model, double netWeight, double maxWeight, boolean isRefrigerated) {
        Response<Boolean> response = transportService.addTruck(licenseNumber, model, netWeight, maxWeight, isRefrigerated);
        if (!response.isSuccess()) throw new RuntimeException(response.getErrorMessage());
    }

    public List<TruckPL> getAllTrucks() {
        Response<List<TruckSL>> response = transportService.getAllTrucks();
        if (!response.isSuccess()) throw new RuntimeException(response.getErrorMessage());
    ...
    public void addDriver(String employeeId, String name, String licenseType, LocalDate licenseExpiry) {
        Response<Boolean> response = transportService.addDriver(employeeId, name, licenseType, licenseExpiry);
        if (!response.isSuccess()) throw new RuntimeException(response.getErrorMessage());
    }

    public List<DriverPL> getAllDrivers() {
        Response<List<DriverSL>> response = transportService.getAllDrivers();
        if (!response.isSuccess()) throw new RuntimeException(response.getErrorMessage());
    ...
    public void addSite(String siteName, String address, String contactPerson, String phoneNumber) {
        Response<Boolean> response = transportService.addSite(siteName, address, contactPerson, phoneNumber);
        if (!response.isSuccess()) throw new RuntimeException(response.getErrorMessage());
    }

    public List<SitePL> getAllSites() {
        Response<List<SiteSL>> response = transportService.getAllSites();
        if (!response.isSuccess()) throw new RuntimeException(response.getErrorMessage());
    ...
    public int createDelivery(LocalDate date, String departureTime, String truckLicense, String driverId, String originSite, List<DestinationPL> destinations, String managerId) {
        ...
        Response<Integer> response = transportService.createDelivery(date, departureTime, truckLicense, driverId, originSite, slDests, managerId);
        if (!response.isSuccess()) throw new RuntimeException(response.getErrorMessage());
        return response.getData();
    }

    public void cancelDelivery(int deliveryId) {
        Response<Boolean> response = transportService.cancelDelivery(deliveryId);
        if (!response.isSuccess()) throw new RuntimeException(response.getErrorMessage());
    }

    public List<DeliveryPL> getAllDeliveries() {
        Response<List<DeliverySL>> response = transportService.getAllDeliveries();
        if (!response.isSuccess()) throw new RuntimeException(response.getErrorMessage());
    ...
    public void replanDelivery(int deliveryId, String newTruckLicense, String newDriverId) {
        Response<Boolean> response = transportService.replanDelivery(deliveryId, newTruckLicense, newDriverId);
        if (!response.isSuccess()) throw new RuntimeException(response.getErrorMessage());
    }
}