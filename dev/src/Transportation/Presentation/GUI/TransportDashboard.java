...
public class TransportDashboard {
    ...
    public TransportDashboard(AppNavigator appNavigator) {
        this.appNavigator = appNavigator;
        transportController = ControllerFactory.getInstance().getTransportController();
    ...
    private VBox createDeliveriesView() {
        ...
        createBtn.setOnAction(e -> {
            try {
                ...
                transportController.createDelivery(datePicker.getValue(), timeField.getText(), truckField.getText(), driverField.getText(), originField.getText(), new ArrayList<>(draftDestinations), managerField.getText());
        ...
        cancelBtn.setOnAction(e -> {
            try {
                transportController.cancelDelivery(Integer.parseInt(modifyIdField.getText()));
                refreshDeliveries();
            } catch (Exception ex) { showAlert("Error", ex.getMessage()); }
        });
    ...
    private Button getReplanBtn(TextField modifyIdField, TextField newTruckField, TextField newDriverField) {
        ...
        replanBtn.setOnAction(e -> {
            try {
                transportController.replanDelivery(Integer.parseInt(modifyIdField.getText()), newTruckField.getText(), newDriverField.getText());
                refreshDeliveries();
    ...
    private void refreshResources() {
        truckList.getItems().setAll(transportController.getAllTrucks().stream().map(t -> t.licenseNumber() + " - " + t.model() + " (Max: " + t.maxWeight() + ")").toList());
        driverList.getItems().setAll(transportController.getAllDrivers().stream().map(d -> d.employeeId() + " - " + d.name() + " (" + d.licenseType() + ")").toList());
    }

    private void refreshSites() {
        siteList.getItems().setAll(transportController.getAllSites().stream().map(SitePL::siteName).toList());
    }
    ...
    private void refreshDeliveries() {
        try {
            deliveryListView.getItems().setAll(transportController.getAllDeliveries().stream().map(d -> "ID: " + d.deliveryId() + " | Date: " + d.date() + " | Truck: " + d.truckLicense() + " | Status: " + d.status()).toList());
