...
public class SqlBranchDAO implements BranchDAO {
    ...
    public List<String> getAllBranchLocations() {
        ...
        } catch (SQLException e) { throw new RuntimeException("Error loading branch locations", e); }
        return locations;
    }
    ...
    public boolean exists(int branchId, String location) {
        ...
        } catch (SQLException e) { throw new RuntimeException("Error checking branch", e); }
    }
    ...
    public List<BranchEntity> getAllBranches() {
        ...
        try (Connection conn = DatabaseManager.getConnection(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) list.add(new BranchEntity(rs.getInt("branchId"), rs.getString("location")));
        } catch (SQLException e) { throw new RuntimeException("Error loading branches", e); }
        return list;
    }
    ...
    public void addUpdateBranch(BranchEntity branch) {
        ...
        try (Connection conn = DatabaseManager.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            pstmt.executeUpdate();
        } catch (SQLException e) { throw new RuntimeException("Error saving branch", e); }
    }
}