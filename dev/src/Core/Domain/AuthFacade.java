...
public class AuthFacade {
    ...
    public Role login(String id, String password) {
        ...
        SessionManager.getInstance().login(id, role);
        return role;
    }

    public Role autoLoginSystemTask(Role role) {
        String employeeId = authDAO.getFirstUserIdByRole(role);
        if (employeeId == null) throw new IllegalArgumentException("CRITICAL: No employee found in database with role " + role);
        SessionManager.getInstance().login(employeeId, role);
        return role;
    }
